...
from ...services.items.core import change_item_ban_status_service, read_private_item_one_admin_service, read_private_item_one_service, read_private_items_many_admin_service
...
@router.get("/{user_id}", response_model=list[ItemOutputPrivate],
            responses={
                401: Responses.RESPONSE_401_UNAUTHORIZED,
                403: Responses.RESPONSE_403_FORBIDDEN,
                404: Responses.RESPONSE_404_NOT_FOUND
            })
def read_private_items_many_admin(session: SessionDep, user_id: int, search: Annotated[ItemSearch, Query()], sort_filter: Annotated[ItemSortFilterPrivate, Query()]):
    return read_private_items_many_admin_service(session, user_id, search, sort_filter)

@router.get("/{item_id}", response_model=ItemOutputPrivate, 
            responses={
                404: Responses.RESPONSE_404_NOT_FOUND
            })
def read_private_item_one_admin(session: SessionDep, item_id: Annotated[int, Path()]):
