from fastapi import APIRouter

from ...core.database import SessionDep
from ...exceptions.core import Responses
from ...models_schemas.users import UserOutputPrivate
from ...services.users import change_user_ban_status_service, read_user_service

router = APIRouter()

# ----- User read (ADMIN) ----- #


@router.get(
    "/{user_id}",
    response_model=UserOutputPrivate,
    responses={404: Responses.RESPONSE_404_NOT_FOUND},
)
async def read_account_admin(session: SessionDep, user_id: int):
    result = await read_user_service(session, user_id)
    return result


# ----- User delete (ADMIN) ----- #


@router.delete(
    "/{user_id}",
    response_model=UserOutputPrivate,
    responses={
        404: Responses.RESPONSE_404_NOT_FOUND,
        409: Responses.RESPONSE_409_CONFLICT,
    },
)
async def ban_account(session: SessionDep, user_id: int):
    user = await change_user_ban_status_service(session, user_id, ban=True)
    return user


@router.post(
    "/{user_id}",
    response_model=UserOutputPrivate,
    responses={
        404: Responses.RESPONSE_404_NOT_FOUND,
        409: Responses.RESPONSE_409_CONFLICT,
    },
)
async def unban_account(session: SessionDep, user_id: int):
    user = await change_user_ban_status_service(session, user_id, ban=False)
    return user
