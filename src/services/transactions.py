...
from ..exceptions.core import ExceptionInvalidValue_409, ExceptionNotFound_404, ObjectType
from ..models_schemas.users import User
from ..models_schemas.transactions import Transaction, TransactionStatus, TransactionType, TransactionInput
...
def change_money(user: User, session: Session, inp: TransactionInput, trans_type: TransactionType) -> Transaction:
    ...
    if trans_type.value is TransactionType.WITHDRAWAL:
        if user_wfu.balance < inp.amount:
            raise ExceptionInvalidValue_409("Account balance", user_wfu.balance - inp.amount)
...
def read_transactions_service(user: User, session: Session, criteria: list[CriterionInput] = []) -> list[Transaction]:
    get_transaction = GetTransaction()
    get_transaction.base_normal()
    get_transaction.get_by("user_id", user.id)
    
    transactions = get_transaction.get_many(session, criteria=criteria, with_for_update=True)
    
    return transactions

def read_transactions_admin_service(session: Session, user_id: int, criteria: list[CriterionInput] = []) -> list[Transaction]:
    get_user = GetUser()
